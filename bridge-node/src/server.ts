...
function genPkce(): PkceData {
  const verifier = base64UrlEncode(crypto.randomBytes(40)).replace(
    /[^a-zA-Z0-9_-]/g,
    ""
  );
...
async function http(
  ...
  const fetchOptions: any = { method, headers, redirect };
  if (body !== undefined) {
...
function parseSetCookie(resp: FetchResponse): string {
...
async function refreshIfNeeded(sess: SessionData): Promise<SessionData> {
...
app.get(
  ...
  async (
    req: Request,
    res: Response<Device[] | { error: string }>
  ): Promise<void> => {
    try {
      ...
      const minimal: Device[] = devices.map((d: any) => ({
...
app.get("/state/:id", async (req: Request, res: Response): Promise<void> => {
  try {
    const sessionId = req.query.session as string;
...
app.post(
  "/command/:id",
  async (
    req: Request<{ id: string }, any, CommandRequest>,
    res: Response
  ): Promise<void> => {
    try {
      const sessionId = req.query.session as string;
