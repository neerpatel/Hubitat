...
GITHUBREPO=https://raw.githubusercontent.com/neerpatel/Hubitat/refs/heads/main/Hubitat/Hubspace
...
for driver in "${APP_FOLDER}"drivers/*.groovy; do
        ...
        if [ -n "$curLoc" ]; then
                ...
                echo "Adding driver: $fname - name: "$name" - namespace: "$ns" - version=${ver:-"(none)"}"
                if [ -n "$ver" ]; then
